...
########################################################################
class LinearRegressionLevel2Model(ModelTemplate):
    """线性回归模型（使用二级因子）"""
...
########################################################################
class LinearRegressionZScorePCModel(ModelTemplate):
    """线性回归模型（LinearRegressionZScoreModel + 仓位控制 position ctrl）"""
...
########################################################################
class LinearRegressionZScoreHigherZeroModel(ModelTemplate):
    """线性回归模型（LinearRegressionZScoreModel + 高于0等频分组）"""
...
########################################################################
class LinearRegressionL1Model(ModelTemplate):
    """线性回归模型（测试用）"""
    ...
    def model_training_and_predict(
        ...
        for i in range(window, len(sorted_dates)):
            ...
            # 训练模型
            try:
                model = LassoCV(alphas=[0.01, 0.1, 1.0], cv=5, fit_intercept=True)
                model.fit(x_train, y_train)
            except Exception as e:
                print(str(e))
                continue
...
########################################################################
class LinearRegressionL2Model(ModelTemplate):
    """线性回归模型（测试用）"""
    ...
    def model_training_and_predict(
        ...
        for i in range(window, len(sorted_dates)):
            ...
            # 训练模型
            try:
                model = RidgeCV(alphas=[0.01, 0.1, 1.0], cv=5, fit_intercept=True)
                model.fit(x_train, y_train)
            except Exception as e:
                print(str(e))
                continue
