...
class KlineDetermination:
    ...
    def limit_up(
        ...
        # -------------------------
        # -1 科创板 20%
        if df["board"].iloc[-1] == "科创板":
            df["limit_rate"] = 0.2
        # -2 创业板
        # 注册制 10% -> 20% | ST 5% -> 20%
        elif df["board"].iloc[-1] == "创业板":
            cond = [
                (df.index >= "2020-08-24"),
        ...
        limit_price = round(pre_close * (1 + df["limit_rate"]), 2).astype("float32")

        return round(df["close"], 2) == limit_price
...
class KLineMetrics(Metrics, KlineDetermination):
    ...
    def __init__(
        ...
        self.metrics["board"] = self.set_board(code)
    ...
    def _ma_close(
        ...
        self.metrics[f"收盘价均线_{self.suffix}_{window}"] = (
            self._calc_rolling(
                self.metrics["close"],
                rolling_window,
                rolling_window
