...
class DataProcessor:
    ...
    def market_value_neutral(
        ...
        """
        市值中性化
        :param factor_data: 待处理的因子数据，支持 DataFrame(多因子) 或 Series(单因子)
        ...
        # --------------------------
        # 1. 去极值
        market_value_processed = cls.percentile(market_value)
        # 2. 标准化
        market_value_processed = cls.standardization(market_value_processed)

        # --------------------------
        # 核心处理逻辑
        # --------------------------
        if isinstance(factor_data, pd.DataFrame):
            return factor_data.apply(lambda col: __market_value_neutral(col))
        elif isinstance(factor_data, pd.Series):
            return __market_value_neutral(factor_data)
        else:
    ...
    def industry_neutral(
        ...
        # --------------------------
        if isinstance(factor_data, pd.DataFrame):
            return factor_data.apply(lambda col: __industry_neutral(col, industry_series))
        elif isinstance(factor_data, pd.Series):
            return __industry_neutral(factor_data, industry_series)
        else:
            raise TypeError('仅支持 DataFrame/Series 类型输入')
    ...
    def restructure_factor(
        ...
        :param factor_series: 因子序列
        :param denominator: 构成因子的分母序列
        ...
        # --------------------------
        # 合并数据并丢弃缺失值
        combined = pd.concat([factor_series, denominator], axis=1)
        ...
        return factor_series.copy().loc[residuals.index] * 0 + residuals
    ...
    def symmetric_orthogonal(
        ...
        # 转换为numpy数组并中心化
        x = factor_data.values.astype(np.float64)
